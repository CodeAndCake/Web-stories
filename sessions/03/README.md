...
* We're dividing our page into sections, so let's create a few empty `section` tags.
	...
	<section></section>
	``` 
* In the first section we'll add a `div`. Inside that, we'll add a **heading** and a **sub-heading**. These will be the most important pieces of information in your page.
	...
	```
* In that same div, underneath the titles, we can add an **image** which could serve as a logo or a profile picture 
...
* We'll have two sections with **textual content**, so let's write something in there.  
	...
	```
* We can add **hyperlinks** to our content.
...
* To center the content sections and make them easier to read, we add `class="content"` to all the `section` elements that contain text. 
